...
bool STITCHING::is_inliner(size_t pic1, size_t pic2, double sx, double sy,
                           const pair<int, int>& kpid, double f = 0) {
  int kpid1, kpid2; tie(kpid1, kpid2) = kpid;
  const auto& kp1 = keypoints[pic1][kpid1], kp2 = keypoints[pic2][kpid2];
  double _sx, _sy;
  if(f > 0) {
    Size sz1 = imgs[pic1].size(), sz2 = imgs[pic2].size();
    double x1, y1;
...
bool STITCHING::is_inliner(size_t pic1, size_t pic2, const Mat& sol,
...
void STITCHING::translation() { 
  ...
  shift.clear();
  shift.resize(pic_num, vector<pair<double,double>>(pic_num));
  //#pragma omp parallel for
  for(size_t pic1 = 0; pic1<pic_num; ++pic1)
    for(size_t pic2 = pic1+1; pic2<pic_num; ++pic2) {
      int best_cnt = 0, best_sx, best_sy;
      const size_t sz = match_pairs[pic1][pic2].size();
      for(int i = 0; i<int(_para[0]); ++i) {
        size_t id1 = rand()%sz;
        int kpid1, kpid2; tie(kpid1, kpid2) = match_pairs[pic1][pic2][id1];
        const auto& kp1 = keypoints[pic1][kpid1], kp2 = keypoints[pic2][kpid2];
        double sx = kp1.x-kp2.x, sy = kp1.y-kp2.y;
        int in_cnt = 0;
        for(size_t id3 = 0; id3<match_pairs[pic1][pic2].size(); ++id3)
          if(id3 != id1) 
            in_cnt += is_inliner(pic1, pic2, sx, sy, 
                                 match_pairs[pic1][pic2][id3]);
        if(in_cnt > best_cnt) {
          best_cnt = in_cnt;
          best_sx = sx;
          best_sy = sy;
      ...
      shift[pic1][pic2] = {best_sx, best_sy};
      cerr << best_sy << " " << best_sy << endl;
      Size sz1 = imgs[pic1].size();
      Size sz2 = imgs[pic2].size();
      const Mat& img0 = imgs[pic1];
      const Mat& img1 = imgs[pic2];
  ...
  for(size_t pic1 = 0; pic1<pic_num; ++pic1)
    for(size_t pic2 = pic1+1; pic2<pic_num; ++pic2) {
      ...
      for(int i = 0; i<int(_para[0]); ++i) {
        ...
        int kpid11, kpid21; tie(kpid11, kpid21) = match_pairs[pic1][pic2][id1];
        int kpid12, kpid22; tie(kpid12, kpid22) = match_pairs[pic1][pic2][id2];
        const auto& kp11 = keypoints[pic1][kpid11];
        const auto& kp21 = keypoints[pic2][kpid21];
        const auto& kp12 = keypoints[pic1][kpid12];
        const auto& kp22 = keypoints[pic2][kpid22];
        ...
        for(size_t id3 = 0; id3<match_pairs[pic1][pic2].size(); ++id3)
          if(id3 != id1) {
            in_cnt1 += is_inliner(pic1, pic2, sx, sy,
                                  match_pairs[pic1][pic2][id3]);
            _para[0] /= 2;
            in_cnt2 += is_inliner(pic1, pic2, sx, sy,
                                  match_pairs[pic1][pic2][id3]);
            _para[0] *= 2;
          }
  ...
  for(size_t pic1 = 0; pic1<pic_num; ++pic1)
    for(size_t pic2 = pic1+1; pic2<pic_num; ++pic2) {
      ...
      for(int i = 0; i<int(_para[0]); ++i) {
        ...
        int kpid11, kpid12; tie(kpid11, kpid12) = match_pairs[pic1][pic2][id1];
        int kpid21, kpid22; tie(kpid21, kpid22) = match_pairs[pic1][pic2][id2];
        int kpid31, kpid32; tie(kpid31, kpid32) = match_pairs[pic1][pic2][id3];
        const auto& kp11 = keypoints[pic1][kpid11];
        const auto& kp21 = keypoints[pic1][kpid21];
        const auto& kp31 = keypoints[pic1][kpid31];
        const auto& kp12 = keypoints[pic2][kpid12];
        const auto& kp22 = keypoints[pic2][kpid22];
        const auto& kp32 = keypoints[pic2][kpid32];
        ...
        for(size_t id3 = 0; id3<match_pairs[pic1][pic2].size(); ++id3)
          if(id3 != id1) {
            in_cnt1 += is_inliner(pic1, pic2, sol,
                                  match_pairs[pic1][pic2][id3]);
            _para[1] /= 2;
            in_cnt2 += is_inliner(pic1, pic2, sol,
                                  match_pairs[pic1][pic2][id3]);
            _para[1] *= 2;
          }
        if(in_cnt1>best_cnt1 || (in_cnt1==best_cnt1 && in_cnt2>best_cnt2)) {
      ...
      const Mat& img1 = imgs[pic1];
      const Mat& img2 = imgs[pic2];
